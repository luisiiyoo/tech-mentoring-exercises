...
const getSelectedCSSClass = (isSelected: boolean) => (isSelected ? "Selected" : '');
...
export const PokerCard: React.FC<CardProps> = ({ indexHand, suit, rank, onSelectCard, isSelected }) => {
  ...
  return (
    <div className={`PokerCard ${cssRank} ${cssSuit} ${cssSelected}`} onClick={() => onSelectCard(indexHand)}>{suit}</div>
  )
}
