...
const CreateGame: React.FC = () => {
  ...
  const handleOnCreateGame = async(event) => {
    try {
      ...
      setIdGame(id);
      store.addNotification(createNotification("success", `Game found.`, 'Success'));
    } catch (err) {
  ...
  return (
    <div className="CreateGame">
      ...
      <div className="CreateGame-Panel">
        <input id="inputPlayerName" placeholder="Player Name" value={playerName} onChange={handleOnChangePlayerName} disabled={disableComponents} />
        <button
